				...
				*Ptr++ = 0;
				...
				else if( CurrentSection && *Start )
				{
					...
					if( Value )
					{
						...
						*Value++ = 0;
						debugf(TEXT("Cmd=%c Start=%s Value=%s"),Cmd, Start,Value);
				        ...
				        FString* Str = NULL;	// Holds the value pointer

						if (Cmd=='+') 
						{
							Str = CurrentSection->FindPair(Start,Value );		
							if (!Str)
