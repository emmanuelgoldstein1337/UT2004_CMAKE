		...
		else
		{
			...
			{ 
			    for( INT i=0; i<ClientConnections.Num() && !Connection; i++ )
			    {
				    ...
				    if( IpAddressRangeMatches( ((UTcpipConnection*)ClientConnections(i))->RemoteAddr, FromAddr ) && ((UTcpipConnection*)ClientConnections(i))->OpenedTime > (GCurrentTime - 60.0) )
				    {
					    SameIPRangeCount++;
					    if( IpAddressMatches( ((UTcpipConnection*)ClientConnections(i))->RemoteAddr, FromAddr ) )
						    SameIPCount++;
				    }
			    }
			}
