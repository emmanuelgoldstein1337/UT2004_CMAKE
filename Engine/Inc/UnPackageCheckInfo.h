...
static DWORD PA_Init[BF_NUM_SUBKEYS] =
{
...
static DWORD SB_Init[BF_NUM_S_BOXES][BF_NUM_ENTRIES] =
{
...
class ENGINE_API FBlowFish
{

private:

	DWORD PA[BF_NUM_SUBKEYS];
	DWORD SB[BF_NUM_S_BOXES][BF_NUM_ENTRIES];

	void Reset(const TCHAR *Key);
	inline void BF_En(BF_Word *x1,BF_Word *x2);
	inline void BF_De(BF_Word *x1,BF_Word *x2);

public:
	void Encrypt(void *Buffer,INT Length, FString Key);
	void Decrypt(void *Buffer,INT Length, FString Key);
};
